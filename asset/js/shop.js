// Boutique + panier + checkout Stripe (avec tailles + option livraison)
import { getSupabase } from './supabase-init.js'

const PRODUCTS = [
  { id: 'p1', name: 'Polo homme manches courtes à liserés contrastés', price: 2160, image: '/img/shop/Polo homme manches courtes à liserés contrastés.png' },
  { id: 'p2', name: 'Bodywarmer matelassé homme',                      price: 3840, image: '/img/shop/Bodywarmer matelassé homme.png' },
  { id: 'p3', name: 'Sweat-shirt zippé capuche contrastée',            price: 4224, image: '/img/shop/Sweat-shirt zippé capuche contrastée.png' },
  { id: 'p4', name: 'Sweat-shirt capuche contrastée homme',            price: 3792, image: '/img/shop/Sweat-shirt capuche contrastée homme.png' },
]
...
const shippingTgl  = document.getElementById('shippingToggle') // ← input checkbox dans le panier
const SHIPPING_CENTS = 1500                                   // 15,00 €
...
function renderProducts() {
  ...
  productsEl.querySelectorAll('[data-add]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const size = (document.getElementById(`size-${id}`)?.value || 'M').toUpperCase()
...
function renderCart() {
  const merchandiseTotal = cart.reduce((sum, i) => {
    const p = PRODUCTS.find(p => p.id === i.id)
    return sum + (p.price * i.quantity)
  }, 0)

  const shipping = shippingTgl?.checked ? SHIPPING_CENTS : 0
  ...
  cartList.innerHTML =
    ...
    (shipping ? `<li><span>Livraison</span><span>${fmt(SHIPPING_CENTS)}</span></li>` : '')
...
shippingTgl?.addEventListener('change', renderCart)
  ...
  try {
    ...
    const res = await fetch('/.netlify/functions/createCheckout', {
      ...
      body: JSON.stringify({
        ...
        shipping: !!shippingTgl?.checked,   // ← on envoie l'option livraison
      }),
    })
    ...
    if (!res.ok) throw new Error(json?.error || `Fonction indisponible (HTTP ${res.status}). Réponse: ${text.slice(0,180)}`)
